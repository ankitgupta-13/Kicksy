...
import { findByEmail } from "../../api/user.api.js";
import { Button, Container, Input, Select } from "../../components/index.js";
...
const Register = () => {
  ...
  const showEmailMessage = (mssg, type) => {
    return (
      <Alert
        onClose={() => {
          onEmailAlertClose();
  ...
  const showMobileMessage = (mssg, type) => {
    return (
      <Alert
        onClose={() => {
          onMobileAlertClose();
  ...
  const handleEmail = ({ email: userEmail }) => {
    const isValidEmail = userEmail.match(/^[\w.-]+@([\w-]+\.)+[\w-]{2,4}$/g);
    ...
    >
      ...
      >
        <div
          ...
          style={{ textAlign: "center", fontWeight: 900, fontSize: "30px" }}
        >
          CREATE ACCOUNT
        </div>
        ...
        <form onSubmit={handleSubmit(handleRegister)} className={style.form}>
          ...
          <div className={style.Input}>
            ...
            {messages.email
              ? showEmailMessage(messages.email, EmailAlertType)
              : ""}
          </div>
          ...
          {messages.mobile
            ? showMobileMessage(messages.mobile, mobileAlertType)
            : ""}
          <div className={style.Input}>
