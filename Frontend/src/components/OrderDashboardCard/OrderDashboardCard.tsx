...
export default function OrderDashboardCard({ order }: { order: any }) {
    ...
    useEffect(() => {
        const fetchFilteredProducts = async () => {
            ...
            const matchedProducts = data.data.filter((product: any) =>
                order.orderItems.some((item: any) => item.product === product._id)
            );
    ...
    const combinedOrderItems = order.orderItems.map((orderItem: any) => {
        const product = orProds.find((prod: any) => prod._id === orderItem.product);
        return {
            ...orderItem,
        ...
        <div className={style.ordercard}>
            ...
            <table style={{ width: "100%" }}>
                ...
                {order.orderItems && Array.isArray(order.orderItems) && order.orderItems.length > 0 ? (
                    combinedOrderItems.map((item: any) => (
                        <tr key={item._id}>
                            <td style={{ textAlign: "center", borderRight: "1px solid #fff" }}>{item.title}</td>
                            <td style={{ textAlign: "center", borderRight: "1px solid #fff" }}>{item.bestPrice}</td>
